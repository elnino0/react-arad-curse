...
function TodosView({id, todos, onTodoComplite, OnAddTodo }) {
  ...
  const render = () =>{
      ...
      if(isAddMod){
        return <div>
        <button onClick={() => {OnAddTodo(todo);setIsAddMod(false);}}>Add</button>
              ...
              <ol>
                  {
                      todos.map((item,index) =>{
                          return <li key={index}>
                              ...
                              <label > { item.complite ? "True" : <div>False <button onClick={e=>(onTodoComplite(id))} >mark as complite</button></div>  } </label>
                          </li>
                      })
                  }
                </ol>
